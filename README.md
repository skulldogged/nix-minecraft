...
- All supported versions of the following:
  - Velocity proxy
- Various tools
  - `fetchModrinthMods` + `update-modrinth-mods`
...
This function takes a lock file generated by `update-modrinth-mods` and produces a directory containing all the mod JARs as a symlink farm. This is the easiest way to manage Modrinth mods declaratively.
...
```shell
nix run github:Infinidoge/nix-minecraft#update-modrinth-mods
```

This reads `mods.toml` and writes `mods.lock.json` with the resolved URLs and hashes. You can also specify custom paths:

```shell
nix run github:Infinidoge/nix-minecraft#update-modrinth-mods -- --manifest path/to/mods.toml --lock path/to/mods.lock.json
```
...
You can pass either the full Modrinth URL or just the modpack slug. After importing, the generated `mods.toml` is yours to edit — you can add, remove, or unpin mods as you see fit, then re-run `update-modrinth-mods` to refresh the lock file.
