...
- All supported versions of the following:
  - Velocity proxy
- Various tools
  - `fetchModrinthMods` + `modrinth-mods`
...
[Source](./pkgs/build-support/fetchModrinthMods.nix)

This function takes a lock file generated by `modrinth-mods` and produces a directory containing all the mod JARs as a symlink farm. This is the easiest way to manage Modrinth mods declaratively.

#### Setup

**1. Create a `mods.toml` manifest** next to your NixOS configuration:
...
lithium = "*"
ferritecore = "6.0.0"     # pin to a specific version number
```

The `[options]` section specifies which Minecraft version and mod loader to resolve against. Under `[mods]`, each key is a Modrinth project slug (from the mod's URL, e.g. `modrinth.com/mod/fabric-api`). The value is either `"*"` for the latest compatible release, or a specific version number string.

**2. Generate the lock file** by running:

```shell
nix run github:skulldogged/nix-minecraft#modrinth-mods
```

This reads `mods.toml` and writes `mods.lock.json` with the resolved URLs and hashes. You can also specify custom paths:

```shell
nix run github:skulldogged/nix-minecraft#modrinth-mods -- --manifest path/to/mods.toml --lock path/to/mods.lock.json
```

**3. Use `fetchModrinthMods`** in your server configuration:

```nix
{
  services.minecraft-servers.servers.my-server = {
    enable = true;
...
To update all mods to their latest compatible versions, simply re-run the update command:

```shell
nix run github:skulldogged/nix-minecraft#modrinth-mods
```

The tool will show which versions changed. Commit the updated `mods.lock.json` to your repository.

To update only specific mods:

```shell
nix run github:skulldogged/nix-minecraft#modrinth-mods -- update fabric-api lithium
```

#### Adding mods

To add one or more mods by Modrinth slug or URL:
...
You can generate a `mods.toml` directly from an existing Modrinth modpack:

```shell
nix run github:skulldogged/nix-minecraft#modrinth-mods -- \
  import-modpack https://modrinth.com/modpack/fabulously-optimized
```

This fetches the modpack's latest version, extracts its mod list, and generates both `mods.toml` (with all mods pinned to the modpack's versions) and `mods.lock.json`.
...
The result of `fetchModrinthMods` exposes a `.drvs` attribute containing the list of individual mod derivations. You can use this to combine Modrinth mods with additional mods:
...
symlinks = {
  mods = pkgs.linkFarmFromDrvs "mods" (
    ...
    ++ [
      (pkgs.fetchurl {
        url = "https://example.com/my-custom-mod.jar";
        hash = "sha256-...";
      })
    ]
  );
...
```shell
nix run github:skulldogged/nix-minecraft#nix-modrinth-prefetch -- versionid
```

(This helper script can also be used in a temporary shell with `nix shell github:skulldogged/nix-minecraft#nix-modrinth-prefetch`)
